...
struct PageUnifiedNetworking: View {
    ...
    */
        ...
        LazyVStack(spacing: 20) {
            if isLoading {
                ...
                HStack(spacing: 20) {
                    ...
                    VStack(spacing: 6) {
                        let total = max(myNetwork.count, 0)
                        let activeMinimum = Int(ceil(Double(total) * 0.20))
                        let activeDisplay = max(activeMinimum, 0)
                        Text("\(activeDisplay)")
                            .font(.system(size: 24, weight: .bold))
    ...
    func fetchUserProfile(userId: Int, completion: @escaping (FullProfile?) -> Void) {
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            if let error = error { print("❌ Request failed:", error); DispatchQueue.main.async { completion(nil) }; return }
            guard let data = data else { DispatchQueue.main.async { completion(nil) }; return }

            let decoder = JSONDecoder()
            // Try 1: decode FullProfile directly
            if let profile = try? decoder.decode(FullProfile.self, from: data) {
                DispatchQueue.main.async { completion(profile) }
                return
            }

            // Try 2: decode legacy UserProfile with single 'name'
            if let up = try? decoder.decode(UserProfile.self, from: data) {
                let parts = up.name.split(separator: " ")
                let first = parts.first.map(String.init) ?? ""
                let last = parts.dropFirst().joined(separator: " ")
                let mapped = FullProfile(
                    user_id: userId,
                    profile_image: up.profile_image,
                    first_name: first,
                    last_name: last,
                    email: up.email,
                    main_usage: nil,
            ...
            // Try 3: minimal JSON with first_name/last_name keys (map as many fields as available)
            if let obj = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
                let first = (obj["first_name"] as? String) ?? ""
                let last = (obj["last_name"] as? String) ?? ""
                let email = (obj["email"] as? String) ?? ""
                let profileImage = (obj["profile_image"] as? String)
                let bio = (obj["bio"] as? String)
                let birthday = (obj["birthday"] as? String)
                let education = (obj["education_level"] as? String)
                let institution = (obj["institution_attended"] as? String)
                let personality = (obj["personality_type"] as? String)
                let connections = obj["connections_count"] as? Int
                let circs = obj["circs"] as? Int
                let history = obj["entrepreneurial_history"] as? String

                // Coerce array-of-strings values robustly
                let coerceStrings: (Any?) -> [String]? = { value in
                    if let v = value as? [String] { return v }
                    if let v = value as? [Any] { return v.compactMap { $0 as? String } }
                    if let v = value as? String { return v.split(separator: ",").map { String($0).trimmingCharacters(in: .whitespaces) } }
                    return nil
                }

                let locations = coerceStrings(obj["locations"])
                let skills = coerceStrings(obj["skillsets"]) ?? coerceStrings(obj["skills"]) // fallback key
                let certificates = coerceStrings(obj["certificates"])
                let clubs = coerceStrings(obj["clubs"])
                let hobbies = coerceStrings(obj["hobbies"])

                let minimal = FullProfile(
                    user_id: userId,
                    ...
                    institution_attended: institution,
                    certificates: certificates,
                    years_of_experience: obj["years_of_experience"] as? Int,
                    ...
                    circs: circs,
                    entrepreneurial_history: history
                )
                DispatchQueue.main.async { completion(minimal) }
                return
            }

            print("❌ Failed to decode user profile: none of the formats matched")
